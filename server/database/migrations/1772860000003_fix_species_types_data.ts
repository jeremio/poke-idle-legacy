...
export default class FixSpeciesTypesData extends BaseSchema {
  async up() {
    ...
    for (const [slug, entry] of Object.entries(POKEMON_TYPES)) {
      const t1 = capitalize(Array.isArray(entry) ? entry[0] : entry)
      const t2 = Array.isArray(entry) ? capitalize(entry[1]) : null
  ...
  async down() {
    // Cannot reliably undo data migration
  }
}
